(function (root) {
  const SAMPLE_SHELVES = [
    ["8B", "1층 꿈뜨락-아동-8B", "아동", "아동 219.2-2020-1-9", "아동 219.2-2020-1-9", "현장 조사 필요", 12.5, 34.5, 11, 11],
  ].map(([shelf_id, label, category, start_call_number, end_call_number, direction, map_x, map_y, map_width, map_height]) => ({
    shelf_id,
    label,
    category,
    start_call_number,
  ...
  function parseShelvesCsv(csvText) {
    return String(csvText || "").trim().split(/\r?\n/).filter(Boolean).slice(1).map((line) => {
      const [shelf_id, label, category, start_call_number, end_call_number, direction, map_x, map_y, map_width, map_height] = line.split(",");
      return {
        shelf_id,
        label,
        category,
  ...
  function usableShelves(shelves) {
    return shelves.length > 0 && shelves.every((shelf) => {
      const start = parseCallNumber(shelf.start_call_number);
      const end = parseCallNumber(shelf.end_call_number);
      return start && end && start.sortKey <= end.sortKey && fitsMap(shelf);
    });
  }
  ...
  function initApp() {
    ...
    function render() {
      ...
      $("#result").textContent = match.shelf ? `${match.shelf.label} · ${match.shelf.direction}` : match.status === "candidates" ? "후보 서가를 선택해 주세요" : "현장 범위 데이터 필요";
      ...
      document.querySelectorAll(".shelf").forEach((node) => node.classList.toggle("active", match.shelf?.shelf_id === node.dataset.shelf));
      ...
      $("#bookList").innerHTML = sorted.map((book, i) => `
        ...
        <span>${escapeHtml(book.parsed?.raw || book.callNumber)} · ${escapeHtml(book.shelf?.shelf_id || "확인 필요")}</span>
        </li>
  ...
  function renderShelves(shelves) {
    ...
    ` + shelves.map((shelf) => `
      <button class="shelf" data-shelf="${escapeHtml(shelf.shelf_id)}" style="left:${shelf.map_x}%;top:${shelf.map_y}%;width:${shelf.map_width}%;height:${shelf.map_height}%">
